...
export class AuthService {
  ...
  async signIn({ username, password }: SignInDto) {
    ...
    if (!user) {
      throw new BadRequestException('Invalid username or password. User not found.')
    }
    ...
    if (!passwordMatch) {
      throw new BadRequestException('Invalid username or password. Incorrect password.')
    }
